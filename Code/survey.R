...
# ################3
#  
# # in experimental groups only
# # unpaired two-samples t-test (Welch)
# t.test(datEducHigh$FB, datEducLow$FB, var.equal = FALSE)
# #sig
# t.test(datScoreHigh$FB, datScoreLow$FB, var.equal = FALSE)
# #not sig
# 
# t.test(datEducLow$FB ~ datEducLow$Match)
# t.test(datEducHigh$FB ~ datEducHigh$Match)
# 
#  
# datEdLowControl = subset(datControl, Education == "Low")
# datEdhighControl = subset(datControl, Education == "High")
...
# # unpaired two-samples t-test (Welch)
# t.test(datEdLowControl$FB, datEdhighControl$FB, var.equal = FALSE)
# #sig
# t.test(datSChighControl$FB, datSCLowControl$FB, var.equal = FALSE)
# #not sig
# 
# mytable <- (datom$FB, datom$Match)
# colnames(mytable) <- c("Group", "College Education Obtained")
# 
# c = chisq.test(table(datom$FB, datom$Match)) 
# 
# 
# # very significant
# xtable()
# # political knowledge affects respondent's preference for the recommended candidate
# t.test(as.numeric(datomEx$Q98) ~ datomEx$Score)  # p-value = 0.03901
# t.test(datomEx$Q98) ~ datomEx$Score)  # p-value = 0.03901
# 
# # Education doesn't affect respondent's preference for the recommended candidate
# t.test(as.numeric(datomEx$Q98) ~ datomEx$Education)  #not sig
# 
#  
# ########################
# 
...
# #################
# 
# sjPlot::tab_xtab(var.row = datCB$group, var.col = datCB$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Group Assignment", 
#                  show.row.prc = TRUE, var.labels = c("Group", "Likelihood to Vote"))
# 
# sjPlot::tab_xtab(var.row = datCB$Education, var.col = datCB$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Education", 
#                  show.row.prc = TRUE, var.labels = c("Education", "Likelihood to Vote"))
# 
# 
# sjPlot::tab_xtab(var.row = datCB$Score, var.col = datCB$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Political Knowledge", 
#                  show.row.prc = TRUE, var.labels = c("Political Knoledge", "Likelihood to Vote"))
# 
# 
# sjPlot::tab_xtab(var.row = datcontrol$Education, var.col = datcontrol$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Education, Control Group",
#                  show.row.prc = TRUE, var.labels = c("Education", "Likelihood to Vote"))
# sjPlot::tab_xtab(var.row = datExp1$Education, var.col = datExp1$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Education, Experimental Group 1", 
#                  show.row.prc = TRUE,  var.labels = c("Education", "Likelihood to Vote"))
# sjPlot::tab_xtab(var.row = datExp2$Education, var.col = datExp2$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Education, Experimental Group 2", 
#                  show.row.prc = TRUE,  var.labels = c("Education", "Likelihood to Vote"))
# 
# 
# sjPlot::tab_xtab(var.row = datcontrol$Score, var.col = datcontrol$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Political Knowledge, Control Group",
#                  show.row.prc = TRUE, var.labels = c("Political Knowledge", "Likelihood to Vote"))
# sjPlot::tab_xtab(var.row = datExp1$Score, var.col = datExp1$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Political Knowledge, Experimental Group 1", 
#                  show.row.prc = TRUE,  var.labels = c("Political Knowledge", "Likelihood to Vote"))
# sjPlot::tab_xtab(var.row = datExp2$Score, var.col = datExp2$FB, title = "A Cross Tabulation of Respondents’ Likelihood to Vote after Taking the Survey and Level of Political Knowledge, Experimental Group 2", 
#                  show.row.prc = TRUE,  var.labels = c("Political Knowledge", "Likelihood to Vote"))
# 
# 
#   
...
# # 
# # datCB <- datCB %>%
# #   mutate(Vote = (FB>6)) # Likelihood to vote >= 70%, 1, or else 0
# # 
# # sjPlot::tab_xtab(var.row = datCB$Education, var.col = datCB$FB), title = "")
# # 
# # 
# # 
# # tabyl(datCB, group)
...
# 
# 
# mytable <- aggregate(FB ~ group + Education, datCB, mean)
# colnames(mytable) <- c("Group", "College Education Obtained", "Likelihood to Vote After Taking the Survey")
# mytable <- subset(mytable, select= c("Group", "College Education Obtained", "Likelihood to Vote After Taking the Survey"))
# xtable(mytable)
# 
# 
# mytable <- aggregate(FB ~ group + Score, datCB, mean)
# colnames(mytable) <- c("Group", "Level of Political Knoledge", "Likelihood to Vote After Taking the Survey")
# mytable <- subset(mytable, select= c("Group", "Level of Political Knoledge", "Likelihood to Vote After Taking the Survey"))
# xtable(mytable)